...
void sr_xml_start_text(ParseContext *ctx, const XML_Char **attrs) {
    const char *const allowed[] = {"id", "text", "width", "height", "size", "color"};
    if (!sr_xml_attrs_allowed(ctx, "text", attrs, allowed, 6)) return;
...
void sr_xml_start_vector(ParseContext *ctx, const XML_Char **attrs) {
    const char *const allowed[] = {"id", "shape", "width", "height", "fill"};
    if (!sr_xml_attrs_allowed(ctx, "vector", attrs, allowed, 5)) return;
    ...
    else SR_XML_FAIL_RETURN(ctx, "vector", "shape", "expected rect or ellipse");
    if (!sr_parse_u32(width, &asset->width) || !asset->width ||
        !sr_parse_u32(height, &asset->height) || !asset->height)
...
void sr_xml_start_object3d(ParseContext *ctx, const XML_Char **attrs) {
    const char *const allowed[]={"id","primitive","material","x","y","z","rotation","rotationX","rotationY","scaleX","scaleY","scaleZ","radius","castShadow","receiveShadow"};
    if(!sr_xml_attrs_allowed(ctx,"object3D",attrs,allowed,15))return;
    ...
    SrObject3D *object=&ctx->scene->objects3d[ctx->scene->object3d_count++];object->id=sr_strdup(id);object->radius=50;object->cast_shadow=true;object->receive_shadow=true;
    ...
    else if(!strcmp(primitive,"plane"))object->primitive=SR_OBJECT_PLANE;else SR_XML_FAIL_RETURN(ctx,"object3D","primitive","expected sphere, box, or plane");
    const char *value=sr_xml_attr(attrs,"material");if(value){object->material_id=sr_strdup(value);if(!object->material_id)SR_XML_FAIL_RETURN(ctx,"object3D","material","out of memory");}
