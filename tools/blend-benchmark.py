...
"""Measure the 22 new color modes against normal blending on the same stack.
...
def inputs(directory):
    fixture = ROOT / 'tests/data-blend-colors.xml'
    tree = ET.parse(fixture)
    changed = 0
    for node in tree.iter():
        if 'blend' in node.attrib:
            node.set('blend', 'normal')
            changed += 1
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='sr-blend-perf-') as temporary:
        ...
        if references[0] == references[1]:
            raise RuntimeError('blend fixture did not change the normal output')
        samples = [[], []]
        ...
        report = {'scene': 'tests/data-blend-colors.xml', 'color_modes': count,
        ...
        for stage in ['composite', 'stage_total']:
            ...
            report['stages'][stage] = {
                'normal_cpu_s': medians[0], 'color_modes_cpu_s': medians[1],
                'delta_percent': (medians[1] / medians[0] - 1) * 100,
                'normal_range_s': [min(values[0]), max(values[0])],
                'color_modes_range_s': [min(values[1]), max(values[1])]}
