...
| Depth cards (2.5D): camera-projected 2D planes (affine fast path, perspective warp ≤ 4096 px side / 8 Mpx), far-to-near run sort, per-sample depth shared with 3D, alpha ≥ 0.5 depth writes, depth-of-field blur (σ ≤ 64 px); standard mode only | `card`, `compositor`, `lighting` | `depth.*` (22 cases, including `depth.thread_invariance`, `depth.projective_matches_analytic`, `depth.crossing_cards`, `depth.depth_of_field`) |
