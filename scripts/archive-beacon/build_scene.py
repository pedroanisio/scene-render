...
def text_width(text, size):
    ...
    return width - scale  # no trailing gap after the last glyph


def check_text(text):
    bad = {c for c in text if not (c.isdigit() or ("A" <= c <= "Z") or c == " ")}
    if bad:
        raise SystemExit(f"text {text!r} uses unsupported glyphs {bad}")
...
def build(render_uhd, render_360):
    ...
    title = "THE ARCHIVE BEACON"
    subtitle = "TRANSMISSION LOG 01"
    card_lines = [
        ("SCENE GENERATED FROM XML", 35, "#EAF6FFFF"),
        ("UHD 3840X2160    360 3840X1920", 21, "#9FD8FFFF"),
        (f"30 FPS    900 FRAMES    SEED {L.SEED}", 21, "#9FD8FFFF"),
        (f"RENDER TIME    UHD {render_uhd}    360 {render_360}", 21,
         "#FFD37AFF"),
        ("SCENE RENDER 1 0 0    CPU    DETERMINISTIC", 14, "#6E8FB0FF"),
    ]
    for text in [title, subtitle, "ARCHIVE STATION ONLINE", "IMPACT WARNING",
                 "SIGNAL A", "SIGNAL B", "SIGNAL C"] + [c[0] for c in card_lines]:
        check_text(text)
    ...
    text_assets = [("titleText", title, 42, "#EAF6FFFF"),
                   ("subtitleText", subtitle, 21, "#7FC8F8FF"),
                   ("onlineText", "ARCHIVE STATION ONLINE", 14, "#7FF3FFFF"),
                   ("warnText", "IMPACT WARNING", 28, "#FF5A5AFF"),
                   ("sigA", "SIGNAL A", 14, "#7FE3FFFF"),
                   ("sigB", "SIGNAL B", 14, "#FFC46BFF"),
                   ("sigC", "SIGNAL C", 14, "#FF8FE0FF")]
    for i, (text, size, color) in enumerate(card_lines):
        text_assets.append((f"card{i}", text, size, color))
    for aid, text, size, color in text_assets:
        emit(f'    <text {attrs(id=aid, text=text, width=text_width(text, size) + 4, height=int(size) + 2, size=size, color=color)}/>')
    emit("  </assets>")

    # ------------------------------------------------------------- materials
    emit("  <materials>")
    emit(f'    <material {attrs(id="hullMat", baseColor="#2F4D72FF", metallic=0.45, roughness=0.5, emissive="#03060CFF")}/>')
    emit(f'    <material {attrs(id="wornMat", baseColor="#4A5058FF", metallic=0.3, roughness=0.75, emissive="#050505FF")}/>')
    ...
    for pid, prim, mat, x, y, r, sx, sy in parts:
        emit(f'    <object3D {attrs(id=pid, primitive=prim, material=mat, x=x, y=y, z=0, radius=r, scaleX=sx, scaleY=sy, castShadow=True)}/>')

    # ------------------------------------------------------------ title
    tw = text_width(title, 42)
    emit(f'    <group {attrs(id="title", x=TITLE_X, y=L.CY, anchorX=tw / 2, anchorY=21, opacity=0)}>')
    ...
    sw = text_width(subtitle, 21)
    emit(f'      <layer {attrs(id="subtitleLayer", asset="subtitleText", x=(tw - sw) / 2, y=70, opacity=0)}>')
    animate("opacity", [(1.4, 0), (2.4, 1)], indent="        ")
    emit("      </layer>")
    emit(f'      <shape {attrs(id="titleRule", shape="rect", width=tw, height=3, x=0, y=56, fill="#4FC3F7CC", blend="add")}/>')
    emit("    </group>")
    ...
    emit("    </shape>")
    ow = text_width("ARCHIVE STATION ONLINE", 14)
    ...
    for did, (cx, cy), sc, color, label, blend, extra, mask, remap, t_in in displays:
        ...
        emit(f'      <layer {attrs(id=f"label{did}", asset=label, x=0, y=-30)}/>')
        emit("    </group>")
    ...
    warn_w = text_width("IMPACT WARNING", 28)
    ...
    emit("      </shape>")
    ys = [ccy - 170, ccy - 70, ccy - 20, ccy + 40, ccy + 130]
    for i, ((text, size, _), y) in enumerate(zip(card_lines, ys)):
        ...
        emit("      </layer>")
    emit(f'      <shape {attrs(id="cardRule", shape="rect", width=760, height=4, x=ccx, y=ccy - 110, anchorX=380, anchorY=2, fill="#4FC3F7FF", blend="add", scaleX=0)}>')
