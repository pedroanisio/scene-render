 ...
 * `blob`) into the pass target; flush resolves what was drawn since the
 * last flush over `frame` (a no-op without antialias3d supersampling) or,
 * with `whole_frame`, the entire supersampled buffer. begin sets *pass to
...
void sr_lighting_draw_object(SrLightingPass *pass, size_t index, bool blob);
